...
def main() -> None:
    ...
    parser.add_argument("--file", help="Retry only failures from specific BibTeX file")
    parser.add_argument(
        "--older-than",
        type=int,
        metavar="DAYS",
    ...
    parser.add_argument(
        "--max-retries", type=int, help="Maximum total retry attempts per entry"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    try:
        ...
        if args.dry_run:
            ...
            if total_success < total_processed:
                print(
                    f"Run again to retry the {total_processed - total_success} remaining failures"
                )
