...
def extract_entry_info(entry: Dict[str, Any]) -> Dict[str, str]:
...
def check_bibkey_format(bibkey: str) -> Tuple[bool, List[str]]:
    ...
    if not match:
        ...
        if re.search(r"[A-Z]", bibkey) and bibkey not in [
            "BID",
            "ID",
        ]:  # Common parser artifacts
...
def check_file(
    filepath: Path,
) -> List[Tuple[str, Dict[str, str], List[str], List[str]]]:
    """
...
def main() -> None:
    ...
    print(
        f"  Violations: {total_violations} ({total_violations / total_entries * 100:.1f}%)"
    )
