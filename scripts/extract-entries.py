...
def extract_entry_key(line):
    """Extract the entry key from a BibTeX entry start line."""
    match = re.match(r'^@\w+\{([^,\s]+)', line.strip())
    return match.group(1) if match else 'unknown'


def find_entry_end(lines, start_idx):
...
def should_skip_entry(entry_type):
...
def extract_and_write_entries(filepath, output_dir):
    ...
    while i < len(lines):
        ...
        if line.startswith('@'):
            ...
            start_idx = i
            entry_key = extract_entry_key(line)
