...
def main() -> None:
    if len(sys.argv) != 3:
        print("Usage: enrich-entry-with-tracking.py <target_file> <entry_key>", file=sys.stderr)
        print("\nThis script enriches a single entry and tracks the result.", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        ...
        print("\n⚠ ACTION REQUIRED:")
        print(f"Run the following agent task:")
        print(f'Task(subagent_type="bibtex-entry-enricher", prompt="Please enrich the BibTeX entry in the file: {tmp_path}")')
        ...
        if is_enriched:
            ...
            track_enrichment(target_file, entry_key, False, error_msg="No enrichment indicators found")
